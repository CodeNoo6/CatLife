...
import 'package:catage/Tensorflow/result.dart';
...
class _HomePageState extends State<HomePage> {
  ...
  Future<Uint8List> preprocessImage(File imageFile) async {
    ...
    // Convert the resized image to RGB format if it's not already in RGB
    ...
    // Convert the image matrix back to a Uint8List
    final bytes = Uint8List.fromList(
        imageMatrix.expand((row) => row.expand((pixel) => pixel)).toList());
  ...
  void navigateToResult() {
    String resultText = _probability > 0.99 ? 'Aprobado' : 'Desaprobado';
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => ResultPage(
          ...
          resultText:
              resultText, // Pasar el texto del resultado a la página de resultados
        ),
      ),
    );
  }
